...
publicRoute.get('/warungs', async (c) => {
  ...
  const warungs = activeOutlets
    .map((outlet) => {
      ...
      const availableProducts = cycles
        .map((cycle) => {
...
// GET /api/public/products - List all available products
publicRoute.get('/products', async (c) => {
  const db = createClient(c.env);

  // Get products that have stock in any warung
  ...
  return c.json({ products: availableProducts });
});
