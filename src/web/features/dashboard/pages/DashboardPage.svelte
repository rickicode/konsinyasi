<script lang="ts">
  ...
  import UrgencyCard from '../components/UrgencyCard.svelte';
...
<section class="flex h-full flex-col bg-milk">
  <header class="px-4 pb-3 pt-safe">
    ...
    <p class="text-sm text-coffee-500">Ringkasan dan prioritas lapangan</p>
  </header>
          ...
          {:else}
            <div class="space-y-3">
              {#each sortedItems as item (item.id)}
                <UrgencyCard {item} distance={distanceFor(item)} {showFinancial} />
              {/each}
            </div>
          {/if}
        </div>
