<script lang="ts">
  ...
  import OwnerSummaryCards from '../components/OwnerSummaryCards.svelte';
...
<section class="flex h-full flex-col bg-milk">
  <header class="px-4 pb-3 pt-safe">
    <h1 class="text-xl font-bold text-coffee-900">Dashboard Owner</h1>
  ...
  <PullToRefresh onRefresh={handleRefresh} class="flex-1">
    <div class="space-y-5 px-4 pb-28 pt-2">
      ...
      {:else if query.isError}
        ...
        />
      {:else if query.data}
        <!-- Owner Summary - Dengan data keuangan lengkap -->
        <OwnerSummaryCards summary={query.data.summary} />

        <!-- Prioritas Warung dengan info keuangan -->
        <div>
